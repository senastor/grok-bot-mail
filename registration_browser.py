...
def getTurnstileToken(log_callback=None, cancel_callback=None, solver_url="http://127.0.0.1:8877"):
    """Get Turnstile token. Tries solver port 8877 first, falls back to manual click."""
    global page
    if page is None:
        raise Exception("页面未就绪，无法执行 Turnstile")

    # === Path A: solver port 8877 ===
    try:
        # Extract sitekey from page (try JS first, fallback to HTML parse)
        sitekey = ""
        try:
            sitekey = str(page.run_js("""
(function() {
  try {
    ...
    if (inputs.length) {
      const p = inputs[0].closest('div');
      if (p && p.getAttribute('data-sitekey')) return p.getAttribute('data-sitekey');
    }
  } catch(e) {}
  return '';
})();
""") or "").strip()
        ...
        if not sitekey:
            # Fallback: regex on page HTML (handles React Server Components escaped JSON)
            try:
                import re as _re
                html = page.html or ""
                # Patterns in regex form, then converted to Python source safely
                patterns = [
                ...
                if log_callback:
                    log_callback(f"[Debug] HTML sitekey extract error: {e}")
        ...
        if sitekey:
            if log_callback:
                log_callback(f"[*] Calling solver {solver_url} for sitekey={sitekey[:16]}...")
            try:
                resp = requests.post(
                    f"{solver_url}/solve",
                    json={
                ...
                if data.get("solved") and data.get("token"):
                    token = str(data["token"]).strip()
                    if log_callback:
                        log_callback(f"[*] Solver returned token (len={len(token)})")
...
(function() {{
  try {{
    ...
    for (const inp of inputs) {{
      try {{ inp.removeAttribute('disabled'); inp.style.display = ''; }} catch(e) {{}}
      try {{ Object.getOwnPropertyDescriptor(HTMLInputElement.prototype, 'value').set.call(inp, {json.dumps(token)}); }} catch(e) {{ inp.value = {json.dumps(token)}; }}
      inp.dispatchEvent(new Event('input', {{bubbles: true}}));
      inp.dispatchEvent(new Event('change', {{bubbles: true}}));
    }}
    if (window.turnstile && typeof window.turnstile.getResponse === 'function') {{
      try {{ window.turnstile.getResponse = function() {{ return {json.dumps(token)}; }}; }} catch(e) {{}}
    }}
...
""")
                        ...
                        if log_callback:
                            log_callback(f"[!] Token inject JS error: {e}")
                    return token
                else:
                    if log_callback:
                        log_callback(f"[!] Solver returned solved={data.get('solved')} error={data.get('error')}")
            except Exception as e:
                if log_callback:
                    log_callback(f"[!] Solver call failed: {e}")
        else:
            if log_callback:
                log_callback("[!] No Turnstile sitekey found on page, falling back to manual")
    except Exception as e:
        if log_callback:
            log_callback(f"[!] Solver path error: {e}")

    # === Path B: fallback to manual click ===
    try:
        page.run_js(
...
def fill_profile_and_submit(timeout=120, log_callback=None, cancel_callback=None):
    ...
    # Eagerly solve Turnstile at start so form can render
    try:
        token0 = getTurnstileToken(log_callback=log_callback, cancel_callback=cancel_callback)
        if log_callback and token0:
            log_callback(f"[*] Eager Turnstile token acquired (len={len(token0)})")
        time.sleep(1.0)
    except Exception as e:
        if log_callback:
            log_callback(f"[Debug] Eager solver failed: {e}")
