...
def imap_create_address(domain=None):
    """Generate a random address on the IMAP forwarding domain.

    For IMAP forwarding, we only need to generate a random address.
    The address exists implicitly via CF Email Routing catch-all -> worker -> Gmail.
    """
    username = generate_username(10)
    target_domain = (domain or config.get("defaultDomains") or "markchain.bond").strip()
    address = f"{username}@{target_domain}"
    # dummy token (unused for IMAP path - we use IMAP session directly)
    return address, "imap-session-shared"


def imap_get_messages(target_email, since=None):
    """Fetch messages from Gmail IMAP inbox targeting a specific address.

    Uses X-GM-RAW search for "to:target_email" against the catch-all Gmail.
    Returns list of dicts: {id, from, subject, date, snippet, raw_body}
    """
    import imaplib
    import email as _email
    from email.header import decode_header

    user = (config.get("imap_user") or config.get("imap_username") or os.environ.get("IMAP_USER", "")).strip()
    pwd = (config.get("imap_password") or config.get("imap_pass") or os.environ.get("IMAP_APP_PASSWORD", "")).strip()
    host = (config.get("imap_host") or "imap.gmail.com").strip()
    folder = (config.get("imap_folder") or "INBOX").strip()

    if not user or not pwd:
        raise Exception("IMAP_USER/IMAP_APP_PASSWORD belum diset di config atau env")

    if not target_email:
        raise Exception("target_email wajib diisi untuk IMAP search")
    ...
    try:
        ...
        # Search recent N days (default 1) for performance
        import datetime as _dt
        days = int(config.get("imap_search_days") or 1)
        since_date = (since or (_dt.date.today() - _dt.timedelta(days=days))).strftime("%d-%b-%Y")

        # Gmail X-GM-RAW supports arbitrary search
        # Find message IDs containing the target_email in To/Cc/Bcc
        query = f'X-GM-RAW "to:{target_email} OR cc:{target_email} OR bcc:{target_email} newer_than:{days}d"'
        ...
        results = []
        for uid in uids[-20:]:  # last 20 max
            try:
                status, msg_data = conn.uid('FETCH', uid, '(RFC822.HEADER RFC822.TEXT)')
                        ...
                        try:
                            ...
                            if payload:
                                ...
                                if isinstance(payload, bytes):
                                    body_text += _re.sub(r'<[^>]+>', ' ', payload.decode(part.get_content_charset() or 'utf-8', errors='ignore'))
                                else:
                                    body_text += _re.sub(r'<[^>]+>', ' ', str(payload))
                        except Exception:
                            pass
                results.append({
...
def imap_get_oai_code(token, email, timeout=180, poll_interval=3,
                     log_callback=None, cancel_callback=None, resend_callback=None):
    """Poll Gmail IMAP for OTP code targeting a specific address."""
    ...
    while _time.time() < deadline:
        ...
        attempts += 1
        try:
            msgs = imap_get_messages(email)
        except Exception as e:
            if log_callback:
                log_callback(f"[Debug] IMAP poll error: {e}")
            _time.sleep(poll_interval)
            continue
        if log_callback:
            log_callback(f"[Debug] IMAP 本轮邮件数量: {len(msgs)}")
        for m in msgs:
            mid = m.get('id')
            if mid in seen_ids:
                continue
            seen_ids.add(mid)
            text = (m.get('subject') or '') + ' ' + (m.get('body') or '') + ' ' + (m.get('snippet') or '')
            if log_callback:
                log_callback(f"[Debug] IMAP 邮件: from={m.get('from','')[:40]} subj={(m.get('subject') or '')[:60]}")
            code = extract_verification_code(text, subject=m.get('subject', ''))
            if code:
                if log_callback:
                    log_callback(f"[*] IMAP 从邮件中提取到验证码: {code}")
                return code
        _time.sleep(poll_interval)
    raise Exception(f"IMAP 等待验证码超时 ({timeout}s)")
